...
keywords={read, write, skip,if,then,else,elif,fi,while,do,od,var,for,fun,local,public,return,import,length,
...
\item in "\lstinline|do..while|" expression the scope of body's \emph{immediate} definitions is implicitly extended to enclose the
  whole expression, thus

  \begin{lstlisting}
     do var i; read (i) while x > 0 of
  \end{lstlisting}
...
\[
  ...
  \trule{\withenv{\Val}{e},\quad\withenv{\Void}{s}}{\withenv{\Weak}{\llang{(do $\;s\;$ while $\;e$); $\;\bot$ }}} &
  \end{array}
  \]
