...
/* Heap is devided on two semi-spaces called active (to-) space and passive
 * (from-) space. */
...
/* 2. Mark phase */
/*   I.e. marking each reachable from the root set via a chain of pointers
 * object as alive. */
/* 3. Copy */
...
/* 1) Static area. */
/*   Globals @__gc_data_end and @__gc_data_start are used to idenfity the begin
 * and the end */
/*   of the static data area. They are defined while generating X86 code in
 * src/X86.lama. */
/* 2) Program stack. */
/*   Globals @__gc_stack_bottom and @__gc_stack_top (see runctime/gc_runtime.s)
 * have to be set */
...
/* You have to implement functions that perform traverse static area
 * (@gc_root_scan_data) */
/* and program stack (@__gc_root_scan_stack, see runtime/gc_runtime.s) as well
 * as a function */
/* (@gc_test_and_copy_root) that checks if a word is a valid heap pointer, and,
 * if so, */
/* call copy-function. Copy-function (@gc_copy) has to move an object into
 * passive semi-space, */
/* rest a forward pointer instead of the object, scan object for pointers, call
 * copying */
/* for each found pointer. */

// The begin and the end of static area (are specified in src/X86.lama fucntion
// genasm)
...
//   finds roots in program stack and calls @gc_test_and_copy_root for each
//   found root
extern void __gc_root_scan_stack();

// You also have to define two functions @__pre_gc and @__post_gc in
// runtime/gc_runtime.s. These auxiliary functions have to be defined in oder to
// correctly set @__gc_stack_top. Note that some of our functions (from
...
// begin of an object. Calling @gc_copy on such values leads to undefined
...
static void init_space(pool *p) {
  p->current = p->begin =
      mmap(NULL, SPACE_SIZE * sizeof(size_t), PROT_READ | PROT_WRITE,
...
#define IS_VALID_HEAP_POINTER(p)                                               \
  (!UNBOXED(p) && from_space.begin <= p && from_space.end > p)

// checks if @p points to the passive (to-) space
#define IN_PASSIVE_SPACE(p) (to_space.begin <= p && to_space.end > p)
...
static void extend_space(pool* p) {
  mremap(p->begin, SPACE_SIZE, SPACE_SIZE * 2, 0);
  p->end += SPACE_SIZE;
  p->size += SPACE_SIZE;
}
...
extern size_t *gc_copy(size_t *obj) {
  ...
  if (TAG(d->tag) == STRING_TAG) {
    *to_space.current++ = d->tag;
    ...
    size_t *position = to_space.current;
    size_t len = LEN(d->tag);
    d->tag = position; // make it forward pointer
  ...
  if (TAG(d->tag) == SEXP_TAG) {
    ...
    *to_space.current++ = s->contents.tag;
    ...
    size_t *position = to_space.current;
    size_t len = LEN(s->contents.tag);
    s->contents.tag = position; // make it forward pointer

    to_space.current += len;
    copy_elements(position, obj, len);
    return position;
  }
  failure("Invalid tag %d\n", d->tag);
}
...
static void *gc(size_t size) {
  ...
  for (size_t i = 0; i < extra_roots.current_free; i++) {
